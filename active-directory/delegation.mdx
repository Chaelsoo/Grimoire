...
**S4U2Self (Service-for-User-to-Self):** Allows a service to request a forwardable service ticket to itself on behalf of any user, without that user's password or prior authentication. Requires the account to have an SPN.

**S4U2Proxy (Service-for-User-to-Proxy):** Allows a service to use a forwardable TGS (obtained from S4U2Self or a real user auth) to request a ticket to a third service. On classic constrained delegation, the KDC validates the target SPN against `msDS-AllowedToDelegateTo`. On RBCD, the KDC validates the source against `msDS-AllowedToActOnBehalfOfOtherIdentity` on the target.
...
**SeEnableDelegationPrivilege:** Normally only Domain Admins can set delegation flags. This privilege grants that ability to non-admin accounts. If a low-priv user has it, they can set `TRUSTED_FOR_DELEGATION` on any account they control, without DA.
...
**Conditions:**
- Write access on the target's `msDS-AllowedToActOnBehalfOfOtherIdentity` (GenericAll, GenericWrite, WriteProperty)
- A controlled account with an SPN — a machine account (sAMAccountName ending in `$`) or a user with an SPN set
